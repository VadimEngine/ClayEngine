...
glm::mat4 CameraXR::getProjectionMatrix() const {
...
glm::mat4 CameraXR::getViewMatrix() const {
    return glm::lookAt(mPosition_, mPosition_ + mForward_, mUp_);
}

void CameraXR::updateCameraVectors() {
...
void CameraXR::move(const glm::vec3 dir, const float step) {
    mPosition_ += (glm::normalize(dir) * step);
}

glm::vec3 CameraXR::getPosition() const {
    return mPosition_;
}

void CameraXR::setPosition(glm::vec3 newPos) {
    mPosition_ = newPos;
}

glm::vec3 CameraXR::getRotation() const {
    return mRotation_;
}

glm::quat CameraXR::getOrientation() const {
    return mOrientation_;
}

void CameraXR::rotateWithJoystick(const glm::vec2& joystickInput) {
...
void CameraXR::updateWithJoystickInput(const glm::vec2& leftJoystick, const glm::vec2& rightJoystick, float moveSpeed, float rotationSpeed, const XrPosef& headPose) {
